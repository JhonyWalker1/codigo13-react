...
export const UserProvider = (props) => {
  ...
  const storeBasket = (product) => {
    ...
    //vmaos a darle la propiedad quantity a lo que es product
    product.quantity = 1;
  ...
  const addOrRemoveProduct = (id, add) => {
    ...
    const products = basket.map((product) => {
      //estamos buscando al producto que tenga el id que le pasemos
      if (product.id === id) {
        if (add) {
          product.quantity +=1;
        } else {
          //debemos validar que la cantidad minima para poder restar debe ser 1
